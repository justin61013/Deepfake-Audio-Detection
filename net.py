...
class SoundStream(nn.Module):
    def __init__(self, C, D, n_q, codebook_size):
        ...
        self.quantizer = ResidualVQ(
            num_quantizers=n_q, dim=200, codebook_size=codebook_size,
...
class Model1(nn.Module):
    def __init__(self, C, D, hidden_units):
        super(Model1, self).__init__()
        self.encoder = Encoder(C, D)    # C = 1, D = 1 output_shape = (batch_size, 250, 1)
        self.fc1 = nn.Linear(250, hidden_units)
        self.relu = nn.ReLU()
...
class Model2(nn.Module):
    def __init__(self, C, D, hidden_units, num_classes=2):
...
class Model3(nn.Module):
    def __init__(self, C, D, n_q, codebook_size, hidden_units, num_classes=2):
        super().__init__()
        ...
        self.dropout1 = nn.Dropout(0.5)
        self.fc1 = nn.Linear(200, hidden_units)
        self.relu = nn.ReLU()
        self.bn1 = nn.BatchNorm1d(hidden_units)
        self.dropout2 = nn.Dropout(0.5)
        self.fc2 = nn.Linear(hidden_units, num_classes)
...
# codebook_size = 32
# hidden_units = 128
# model2 = Model2(C, D, n_q, codebook_size, hidden_units)
