...
train_dataset = NSynthDataset(audio_dir=r"C:\Users\sean8_q3zpzqf\Desktop\Deepfake Audio Detection\dataset\train\audio1")
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, collate_fn=collate_fn, num_workers=2)
sr = train_dataset.sr

valid_dataset = NSynthDataset(audio_dir=r"C:\Users\sean8_q3zpzqf\Desktop\Deepfake Audio Detection\dataset\valid\audio1")
valid_loader = DataLoader(valid_dataset, batch_size=BATCH_SIZE, collate_fn=collate_fn, num_workers=2)

test_dataset = NSynthDataset(audio_dir=r"C:\Users\sean8_q3zpzqf\Desktop\Deepfake Audio Detection\dataset\test\audio1")
...
def adversarial_g_loss(features_stft_disc_G_x, features_wave_disc_G_x, lengths_stft, lengths_wave):
    ...
    stft_loss = F.relu(1-features_stft_disc_G_x[-1]).sum(dim=3).squeeze()/lengths_stft[-1]
    wave_loss = torch.cat([F.relu(1-features_wave_disc_G_x[key][-1]).sum(dim=2).squeeze()/lengths_wave[key][-1] for key in wave_disc_names])
...
def adversarial_d_loss(features_stft_disc_x, features_wave_disc_x, features_stft_disc_G_x, features_wave_disc_G_x, lengths_stft, lengths_wave):
    ...
    real_wave_loss = torch.stack([F.relu(1-features_wave_disc_x[key][-1]).sum(dim=-1).squeeze()/lengths_wave[key][-1] for key in wave_disc_names], dim=1)
    real_loss = torch.cat([real_stft_loss.view(-1,1), real_wave_loss], dim=1).mean()
    
    generated_stft_loss = F.relu(1+features_stft_disc_G_x[-1]).sum(dim=-1).squeeze()/lengths_stft[-1]
    generated_wave_loss = torch.stack([F.relu(1+features_wave_disc_G_x[key][-1]).sum(dim=-1).squeeze()/lengths_wave[key][-1] for key in wave_disc_names], dim=1)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, N_EPOCHS+1):
        ...
        for x, lengths_x in tqdm(train_loader):
            ...
            s_x = torch.stft(x.squeeze(1), n_fft=1024, hop_length=256, window=torch.hann_window(window_length=1024, device=device), return_complex=False).permute(0, 3, 1, 2)
            lengths_s_x = 1 + torch.div(lengths_x, 256, rounding_mode="floor")
            s_G_x = torch.stft(G_x.squeeze(1), n_fft=1024, hop_length=256, window=torch.hann_window(window_length=1024, device=device), return_complex=False).permute(0, 3, 1, 2)
        ...
        with torch.no_grad():
            ...
            if valid_loss_g < best_val_loss:
                best_model = soundstream.state_dict().copy()
                # save model
                torch.save(best_model, "weight/best_model.pt")
        ...
        with torch.no_grad():
            ...
            # save model
            torch.save(best_model, "weight/last_model.pt")
